...
import type { SuggestionReviewFormState } from "@/lib/admin/suggestion-form";
import { useTranslations } from "@/components/i18n/LocaleProvider";

type AdminSuggestionFormFieldsProps = {
  form: SuggestionReviewFormState;
  parentOptions: string[];
  onChange: (form: SuggestionReviewFormState) => void;
};

export default function AdminSuggestionFormFields({
  form,
  parentOptions,
  onChange,
}: AdminSuggestionFormFieldsProps) {
    ...
    <div className="mt-6 space-y-5">
      <label className="block space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={() => onChange({ ...form, isSub: false, parentName: null })}
          ...
          <button
            ...
            onClick={() =>
              onChange({
                ...form,
                isSub: true,
                parentName: form.parentName ?? parentOptions[0] ?? null,
              })
            }
            className={`rounded-2xl border px-4 py-3 text-sm font-medium transition ${
              form.isSub
        ...
        <label className="block space-y-2">
          ...
          <select
            ...
            onChange={(event) =>
              onChange({
                ...form,
                parentName: event.target.value || null,
              })
            }
      ...
      >
        ...
        <div>
          ...
          {form.isSub && form.parentName ? (
            <p className="text-xs text-slate-500">
              {t("categories.under")} {form.parentName}
            </p>
          ) : null}
        </div>
      </div>
